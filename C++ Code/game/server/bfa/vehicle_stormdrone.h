...
protected:
	...
	CPropVehicleStormDrone *GetManhack( void );
};

class CPropVehicleStormDrone :  public CBaseProp, public IDrivableVehicle
{
	DECLARE_CLASS( CPropVehicleStormDrone, CBaseProp );
public:
	CPropVehicleStormDrone *m_pNext;

	static CPropVehicleStormDrone *GetManhackVehicle();


	DECLARE_DATADESC();
	DECLARE_SERVERCLASS();

	CPropVehicleStormDrone( void );

	~CPropVehicleStormDrone( void );

	CNPC_STORMDRONE *GetManhack();
...
private:
	...
	CHandle<CNPC_STORMDRONE>		m_hManhack;

	CHandle<CNPC_STORMDRONE>		m_hSetOfManhacks[NUMBER_OF_MAX_CONTROLLABLE_MANHACKS];
