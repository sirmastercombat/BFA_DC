...
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CWeapon_StormDrone )
...
acttable_t	CWeapon_StormDrone::m_acttable[] = 
{
	{ ACT_RANGE_ATTACK1, ACT_RANGE_ATTACK_SLAM, true },
};

IMPLEMENT_ACTTABLE(CWeapon_StormDrone);


IMPLEMENT_SERVERCLASS_ST( CWeapon_StormDrone, DT_Weapon_Manhack)
	SendPropInt(SENDINFO(m_iManhackDistance), SPROP_CHANGES_OFTEN),
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS( weapon_manhack, CWeapon_StormDrone );
PRECACHE_WEAPON_REGISTER(weapon_manhack);



void CWeapon_StormDrone::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	...
	switch( pEvent->event )
	{
		...
		case EVENT_WEAPON_THROW:
			if (CPropVehicleStormDrone::GetManhackVehicle()!=NULL)
			{
				if (m_bHasFreeSlot)	
				{
...
void CWeapon_StormDrone::GetControlPanelInfo( int PanelIndex, const char *&pPanelName)
{
	pPanelName = NULL; //"manhack_screen";
}

CWeapon_StormDrone::CWeapon_StormDrone(void)
{
//	m_hManhack = NULL;
	m_bIsDrawing=false;
	m_bIsDoingShit=false;
...
// Output  :
//------------------------------------------------------------------------------
void CWeapon_StormDrone::ItemBusyFrame( void )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
// Output  :
//------------------------------------------------------------------------------
void CWeapon_StormDrone::ItemPostFrame( void )
{
	m_bHasAmmo = false;
	...
	if (pOwner && !m_bIsDoingShit && !m_bIsDrawing && !m_bIsDoingShitToo && HasAnyAmmo() )
	{
		...
		if( pOwner->m_nButtons & IN_ATTACK )
		{
			if ( m_bHasAmmo && (CPropVehicleStormDrone::GetManhackVehicle() == NULL || m_bHasFreeSlot ) && !m_bIsDoingController)
			{
				PrimaryAttack();
			}
			else 
...
bool CWeapon_StormDrone::FindVehicleManhack()
{
	/*if (m_hManhack==NULL)
	{
...
void CWeapon_StormDrone::TellManhacksToGoThere() 
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
	CPropVehicleStormDrone *pVehicleManhack = CPropVehicleStormDrone::GetManhackVehicle(); //dynamic_cast<CPropVehicleStormDrone*>((CBaseEntity*)m_hManhack);
...
void CWeapon_StormDrone::CallManhacksBack()
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
	CPropVehicleStormDrone *pVehicleManhack = CPropVehicleStormDrone::GetManhackVehicle(); //dynamic_cast<CPropVehicleStormDrone*>((CBaseEntity*)m_hManhack);
	if (pVehicleManhack!=NULL && pPlayer != NULL) 
...
void CWeapon_StormDrone::Precache( void )
{
	BaseClass::Precache();
...
// Output  :
//------------------------------------------------------------------------------
void CWeapon_StormDrone::PrimaryAttack()
{
	if ( !m_bHasFreeSlot )
	{
		return;
...
// Output  :
//------------------------------------------------------------------------------
void CWeapon_StormDrone::SecondaryAttack()
{
	if ( CPropVehicleStormDrone::GetManhackVehicle() == NULL )
	{
		return;
...
void CWeapon_StormDrone::UpdateControllerPanel()
{
	if (GlobalEntity_GetState("manhacks_not_controllable") == GLOBAL_ON )
	{
	...
	CPropVehicleStormDrone *pManhack = CPropVehicleStormDrone::GetManhackVehicle(); //dynamic_cast<CPropVehicleStormDrone*>((CBaseEntity*)m_hManhack);
...
void CWeapon_StormDrone::WeaponIdle( void )
{
	/*bool hasAmmo = false;
	CBaseCombatCharacter *pOwner  = GetOwner();
...
void CWeapon_StormDrone::EnableManhackSubModel(bool bEnable)
{
...
bool CWeapon_StormDrone::Deploy( void )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	if ( !pOwner )
	{
	...
	CPropVehicleStormDrone *pManhack = CPropVehicleStormDrone::GetManhackVehicle();

	if (pManhack) //m_hManhack)
	{
		// = dynamic_cast<CPropVehicleStormDrone*>((CBaseEntity*)m_hManhack);
	...
	if (GlobalEntity_GetState("manhacks_not_controllable") == GLOBAL_ON )
	{
		if (m_bHadControllable)
		{
			m_iManhackHintTimeShown=0;
			CBasePlayer *pPlayer = ToBasePlayer( pOwner );
			if ( pPlayer != NULL && (CPropVehicleStormDrone::GetManhackVehicle() != NULL) )
			{
				UTIL_HudHintText( pPlayer, "#HLSS_Hint_ManhackSend" );
				m_iManhackHintTimeShown++;
			}
...
bool CWeapon_StormDrone::HasAnyAmmo( void )
{
	FindVehicleManhack();
...
void CWeapon_StormDrone::WeaponSwitch( void )
{
	CBaseCombatCharacter *pOwner  = GetOwner();
	if (pOwner==NULL) return;
	pOwner->SwitchToNextBestWeapon( pOwner->GetActiveWeapon() );

	if (CPropVehicleStormDrone::GetManhackVehicle() == NULL && pOwner->GetAmmoCount(m_iPrimaryAmmoType) <= 0)
	{
		pOwner->ClearActiveWeapon();
	}
}

void CWeapon_StormDrone::DecrementAmmo( CBaseCombatCharacter *pOwner )
{
	pOwner->RemoveAmmo( 1, m_iPrimaryAmmoType ); //Secondary

}

bool CWeapon_StormDrone::Reload( void )
{
	//Deploy();
	WeaponIdle();
	return true;
...
bool CWeapon_StormDrone::CreateControllableVehicleManhack( CBasePlayer *pPlayer )
{
	/*Vector vecOrigin;
	QAngle vecAngles;
...
bool CWeapon_StormDrone::CreateControllableNPCManhack( CBasePlayer *pPlayer )
{
	/*Vector vecOrigin;
	QAngle vecAngles;
...
bool CWeapon_StormDrone::HasNPCManhack()
{
	CPropVehicleStormDrone *pManhack = CPropVehicleStormDrone::GetManhackVehicle(); //dynamic_cast<CPropVehicleStormDrone*>((CBaseEntity*)m_hManhack);
	if (pManhack!=NULL) 
	{	
...
bool CWeapon_StormDrone::HasFreeSlot()
{
	CPropVehicleStormDrone *pManhack = CPropVehicleStormDrone::GetManhackVehicle(); //dynamic_cast<CPropVehicleStormDrone*>((CBaseEntity*)m_hManhack);
	if (pManhack!=NULL)
	{
...
int CWeapon_StormDrone::NumberOfManhacks()
{
	CPropVehicleStormDrone *pManhack = CPropVehicleStormDrone::GetManhackVehicle(); //dynamic_cast<CPropVehicleStormDrone*>((CBaseEntity*)m_hManhack);
	if (pManhack!=NULL)
	{
...
void CWeapon_StormDrone::DriveControllableManhack()
{
	CPropVehicleStormDrone *pManhack = CPropVehicleStormDrone::GetManhackVehicle(); //dynamic_cast<CPropVehicleStormDrone*>((CBaseEntity*)m_hManhack);
	if (pManhack!=NULL) 
	{
...
float CWeapon_StormDrone::CalcViewmodelBob( void )
{
