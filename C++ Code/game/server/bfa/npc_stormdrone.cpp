...
ConVar	STORMDRONE_blend_collision("hlss_STORMDRONE_blend_collision", "16");

// When the engine is running and the manhack is operating under power
// we don't let gravity affect him.
#define STORMDRONE_GRAVITY 0.000

#define STORMDRONE_GIB_COUNT			5 
#define STORMDRONE_INGORE_WATER_DIST	384

// Sound stuff
#define STORMDRONE_PITCH_DIST1		512
#define STORMDRONE_MIN_PITCH1		(100)
#define STORMDRONE_MAX_PITCH1		(160)
#define STORMDRONE_WATER_PITCH1	(85)
#define STORMDRONE_VOLUME1			0.35

#define STORMDRONE_PITCH_DIST2		400
#define STORMDRONE_MIN_PITCH2		(85)
#define STORMDRONE_MAX_PITCH2		(190)
#define STORMDRONE_WATER_PITCH2	(90)

#define STORMDRONE_NOISEMOD_HIDE 5000

#define STORMDRONE_BODYGROUP_BLADE	1
#define STORMDRONE_BODYGROUP_BLUR	2
#define STORMDRONE_BODYGROUP_OFF	0
#define STORMDRONE_BODYGROUP_ON	1

// ANIMATION EVENTS
#define STORMDRONE_AE_START_ENGINE			50
#define STORMDRONE_AE_DONE_UNPACKING		51
#define STORMDRONE_AE_OPEN_BLADE			52

//#define STORMDRONE_GLOW_SPRITE	"sprites/laserdot.vmt"
#define STORMDRONE_GLOW_SPRITE	"sprites/glow1.vmt"

#define	STORMDRONE_CHARGE_MIN_DIST	200


//TERO: this was simply 1.0f in Valve's code. However, it can be hard to break vents when so.
#define HLSS_STORMDRONE_COLLISION_SCALAR 3.0f

ConVar	sk_stormdrone_health( "sk_stormdrone_health","0");
ConVar	sk_stormdrone_melee_dmg( "sk_stormdrone_melee_dmg","0");
ConVar	sk_stormdrone_v2( "sk_stormdrone_v2","1");
ConVar  STORMDRONE_debug_collision("hlss_STORMDRONE_debug_collision","0");
...
// Private activities.
//-----------------------------------------------------------------------------
extern Activity ACT_MANHACK_UNPACK;
...
{
	COND_STORMDRONE_START_ATTACK = LAST_SHARED_CONDITION,	// We are able to do a bombing run on the current enemy.
};
...
{
	SCHED_STORMDRONE_ATTACK_HOVER = LAST_SHARED_SCHEDULE,
	SCHED_STORMDRONE_DEPLOY,
	SCHED_STORMDRONE_REGROUP,
	SCHED_STORMDRONE_SWARM_IDLE,
	SCHED_STORMDRONE_SWARM,
	SCHED_STORMDRONE_SWARM_FAILURE,
};
...
enum ManhackTasks
{
	TAsk_stormdrone_HOVER = LAST_SHARED_TASK,
	TAsk_stormdrone_UNPACK,
	TAsk_stormdrone_FIND_SQUAD_CENTER,
	TAsk_stormdrone_FIND_SQUAD_MEMBER,
	TAsk_stormdrone_MOVEAT_SAVEPOSITION,
};

BEGIN_DATADESC( CNPC_STORMDRONE )
...
LINK_ENTITY_TO_CLASS( npc_stormdone, CNPC_STORMDRONE );

IMPLEMENT_SERVERCLASS_ST(CNPC_STORMDRONE, DT_npc_stormdone)
...
// Output  :
//------------------------------------------------------------------------------
CNPC_STORMDRONE::CNPC_STORMDRONE()
{
#ifdef _DEBUG
...
//-----------------------------------------------------------------------------
Class_T	CNPC_STORMDRONE::Classify(void)
{
...
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::Event_Dying(void)
{
	DestroySmokeTrail();
	SetHullSizeNormal();
...
//-----------------------------------------------------------------------------
Disposition_t  CNPC_STORMDRONE::IRelationType( CBaseEntity *pEnemy )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::GatherConditions()
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::PrescheduleThink( void )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::DeathSound( const CTakeDamageInfo &info )
{
	StopSound("npc_stormdone.Stunned");
	CPASAttenuationFilter filter2( this, "npc_stormdone.Die" );
	EmitSound( filter2, entindex(), "npc_stormdone.Die" );
}
...
//-----------------------------------------------------------------------------
bool CNPC_STORMDRONE::ShouldGib( const CTakeDamageInfo &info )
{
	return ( m_bGib );
}
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::Event_Killed( const CTakeDamageInfo &info )
{
	// turn off the blur!
	SetBodygroup( STORMDRONE_BODYGROUP_BLUR, STORMDRONE_BODYGROUP_OFF );
...
void CNPC_STORMDRONE::HitPhysicsObject( CBaseEntity *pOther )
{
...
//-----------------------------------------------------------------------------
#define STORMDRONE_SMASH_SPEED 500.0	// How fast a manhack must slam into something to take full damage
void CNPC_STORMDRONE::TakeDamageFromPhyscannon( CBasePlayer *pPlayer )
{
	...
	float flFactor = flSpeed / STORMDRONE_SMASH_SPEED;
...
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::TakeDamageFromVehicle( int index, gamevcollisionevent_t *pEvent )
{
...
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::TakeDamageFromPhysicsImpact( int index, gamevcollisionevent_t *pEvent )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
#define STORMDRONE_SMASH_TIME	0.35		// How long after being thrown from a physcannon that a manhack is eligible to die from impact
void CNPC_STORMDRONE::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	// Wake us up
	if ( m_spawnflags & SF_STORMDRONE_PACKED_UP )
	{
	...
	CBasePlayer *pPlayer = HasPhysicsAttacker( STORMDRONE_SMASH_TIME );
	if( pPlayer )
	{
		if (!pHitEntity)
		{
...
void CNPC_STORMDRONE::VPhysicsShadowCollision( int index, gamevcollisionevent_t *pEvent )
{
	int otherIndex = !index;
	CBaseEntity *pOther = pEvent->pEntities[otherIndex];
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::CrashTouch( CBaseEntity *pOther )
{
...
// Create smoke trail!
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::CreateSmokeTrail()
{
	if ( HasSpawnFlags( SF_STORMDRONE_NO_DAMAGE_EFFECTS ) )
		return;
...
void CNPC_STORMDRONE::DestroySmokeTrail()
{
...
// Output :
//-----------------------------------------------------------------------------
int	CNPC_STORMDRONE::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
...
#define STORMDRONE_PHYS_SEARCH_SIZE		64
#define	STORMDRONE_PHYSICS_SEARCH_RADIUS	128

		CBaseEntity *pList[ STORMDRONE_PHYS_SEARCH_SIZE ];

		Vector attackDir = info.GetDamageForce();
		VectorNormalize( attackDir );

		Vector testCenter = GetAbsOrigin() + ( attackDir * STORMDRONE_PHYSICS_SEARCH_RADIUS );
		Vector vecDelta( STORMDRONE_PHYSICS_SEARCH_RADIUS, STORMDRONE_PHYSICS_SEARCH_RADIUS, STORMDRONE_PHYSICS_SEARCH_RADIUS );

		int count = UTIL_EntitiesInBox( pList, STORMDRONE_PHYS_SEARCH_SIZE, testCenter - vecDelta, testCenter + vecDelta, 0 );
	...
	if ( nRetVal )
	{
		if ( m_iHealth > 0 )
		{
			if ( info.GetDamageType() & DMG_CLUB )
			{
				SetEyeState( STORMDRONE_EYE_STATE_STUNNED );
			}
...
// Purpose:
//------------------------------------------------------------------------------
bool CNPC_STORMDRONE::CorpseGib( const CTakeDamageInfo &info )
{
...
// Output :
//-----------------------------------------------------------------------------
int	CNPC_STORMDRONE::OnTakeDamage_Dying( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::OnStateChange( NPC_STATE OldState, NPC_STATE NewState )
{
	if( m_vNoiseMod.z == STORMDRONE_NOISEMOD_HIDE && !(m_spawnflags & SF_NPC_WAIT_FOR_SCRIPT) && !(m_spawnflags & SF_STORMDRONE_PACKED_UP) )
	{
...
// Input  : Type - 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::HandleAnimEvent( animevent_t *pEvent )
{
	Vector vecNewVelocity;
	switch( pEvent->event )
	{
	case STORMDRONE_AE_START_ENGINE:
		StartEye();
		StartEngine( true );
		m_spawnflags &= ~SF_STORMDRONE_PACKED_UP;

		// No bursts until fully unpacked!
		m_flNextBurstTime = gpGlobals->curtime + FLT_MAX;
		break;

	case STORMDRONE_AE_DONE_UNPACKING:
		m_flNextBurstTime = gpGlobals->curtime + 2.0;
		break;

	case STORMDRONE_AE_OPEN_BLADE:
...
//-----------------------------------------------------------------------------
bool CNPC_STORMDRONE::IsFlyingActivity( Activity baseAct )
{
...
// Input  : Type - 
//-----------------------------------------------------------------------------
Activity CNPC_STORMDRONE::NPC_TranslateActivity( Activity baseAct )
{
...
int CNPC_STORMDRONE::TranslateSchedule( int scheduleType ) 
{
	...
	case SCHED_MELEE_ATTACK1:
		{
			return SCHED_STORMDRONE_ATTACK_HOVER;
			break;
		}
	case SCHED_BACK_AWAY_FROM_ENEMY:
		{
			return SCHED_STORMDRONE_REGROUP;
			break;
		}
	...
	case SCHED_ALERT_FACE:
		{
			...
			{
				return SCHED_STORMDRONE_SWARM_IDLE;
			}
			else
			{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::MaintainGroundHeight( void )
{
...
// Input  : flInterval - 
//-----------------------------------------------------------------------------
bool CNPC_STORMDRONE::OverrideMove( float flInterval )
{
	SpinBlades( flInterval );
		
	// Don't execute any move code if packed up.
	if( HasSpawnFlags(SF_STORMDRONE_PACKED_UP|SF_STORMDRONE_CARRIED) )
		return true;
	...
	if (!m_bControllable)
	{
		...
		if ( !HasSpawnFlags( SF_STORMDRONE_NO_DANGER_SOUNDS ) && !m_bHeld )
		{
...
void CNPC_STORMDRONE::SetControllable(bool bControllable)
{
	m_bControllable=bControllable;	
}

void CNPC_STORMDRONE::ShowRedGlow(bool bHide)
{
...
/*void CNPC_STORMDRONE::SetHeadYaw(float HeadYaw)
{
	m_fHeadYaw=HeadYaw;
	//SetBoneController( 0, m_fHeadYaw );
}*/

void CNPC_STORMDRONE::MoveUpDown(float direction ) 
{
	trace_t tr;
	UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + Vector(0,0,m_vForceVelocity.z * STORMDRONE_blend_collision.GetFloat()), MoveCollisionMask(), this, COLLISION_GROUP_NONE, &tr);
...
void CNPC_STORMDRONE::MoveForwardBack(float direction, QAngle angManhackEye) 
{
	Vector forward;
	...
	if (direction>=0)
	{
		...
		AI_TraceHull( GetAbsOrigin(), 
					  GetAbsOrigin() + velocity * STORMDRONE_blend_collision.GetFloat(), 
					  GetHullMins(), 
					  GetHullMaxs(), 
					  MoveCollisionMask(),  
					  this, 
		...
		if (tr.fraction!=1.0)
		{
			if (STORMDRONE_debug_collision.GetInt() == 1)
			{
				NDebugOverlay::Line( GetAbsOrigin(), GetAbsOrigin() + m_vForceVelocity * STORMDRONE_blend_collision.GetFloat(), 255, 0, 255, 0, 5 );
			}	
			if (STORMDRONE_debug_collision.GetInt() == 2)
			{
...
Vector CNPC_STORMDRONE::GetManhackView()
{
	return GetAbsOrigin(); // + ( m_vCollisionView * STORMDRONE_blend_view.GetFloat() );
}

void CNPC_STORMDRONE::ComeBackToPlayer(CBasePlayer *pPlayer, float fCallBackTime)
{
	...
	if (pPlayer && m_fForceMoveTime - gpGlobals->curtime < 2.0f)
	{
		...
		DevMsg("npc_stormdone, controllable: Coming back to player\n");
	}
}

void CNPC_STORMDRONE::GoThere(CBasePlayer *pPlayer, float fGoThereTime)
{
		m_bControllable = false;
		...
		DevMsg("npc_stormdone, controllable: Going where player eyes are pointing\n");
}
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::TurnHeadRandomly(float flInterval )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::MoveToTarget(float flInterval, const Vector &vMoveTarget)
{
	if (flInterval <= 0)
	{
		return;
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_STORMDRONE::MoveCollisionMask(void)
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::Splash( const Vector &vecSplashPos )
{
	CEffectData	data;
...
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::ComputeSliceBounceVelocity( CBaseEntity *pHitEntity, trace_t &tr )
{
...
//-----------------------------------------------------------------------------
bool CNPC_STORMDRONE::IsHeldByPhyscannon( )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::Slice( CBaseEntity *pHitEntity, float flInterval, trace_t &tr )
{
	...
	float flDamage = sk_stormdrone_melee_dmg.GetFloat() * flInterval;
	...
	else if ( pHitEntity->IsNPC() && HasPhysicsAttacker( STORMDRONE_SMASH_TIME ) )
	{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::Bump( CBaseEntity *pHitEntity, float flInterval, trace_t &tr )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::CheckCollisions(float flInterval)
{
	...
	vecTraceDir *= flInterval * HLSS_STORMDRONE_COLLISION_SCALAR;
	if ( IsHeldByPhyscannon() )
	{
	...
	if ( (tr.fraction != 1.0 || tr.startsolid) && tr.m_pEnt)
	{
		...
		/*if (m_bControllable)
		{
			...
			/*if (Dist >= 0 && Dist < STORMDRONE_blend_collision.GetFloat())
			{
				m_vCollisionView = 	vecTraceDir * (STORMDRONE_blend_collision.GetFloat() - Dist);
...
void CNPC_STORMDRONE::PlayFlySound(void)
{
	float flEnemyDist;
	...
	if( m_flWaterSuspendTime > gpGlobals->curtime )
	{
		...
		if( m_bDirtyPitch )
		{
			m_nEnginePitch1 = STORMDRONE_WATER_PITCH1;
			m_flEnginePitch1Time = gpGlobals->curtime + 0.5f;
			m_nEnginePitch2 = STORMDRONE_WATER_PITCH2;
	...
	{
		if( flEnemyDist < STORMDRONE_PITCH_DIST1 )
		{
			// recalculate pitch.
			int iPitch1, iPitch2;
			float flDistFactor;

			flDistFactor = min( 1.0, 1 - flEnemyDist / STORMDRONE_PITCH_DIST1 ); 
			iPitch1 = STORMDRONE_MIN_PITCH1 + ( ( STORMDRONE_MAX_PITCH1 - STORMDRONE_MIN_PITCH1 ) * flDistFactor); 

			// NOTE: STORMDRONE_PITCH_DIST2 must be < STORMDRONE_PITCH_DIST1
			flDistFactor = min( 1.0, 1 - flEnemyDist / STORMDRONE_PITCH_DIST2 ); 
			iPitch2 = STORMDRONE_MIN_PITCH2 + ( ( STORMDRONE_MAX_PITCH2 - STORMDRONE_MIN_PITCH2 ) * flDistFactor); 
		...
		{
			m_nEnginePitch1 = STORMDRONE_MIN_PITCH1;
			m_flEnginePitch1Time = gpGlobals->curtime + 0.1f;
			m_nEnginePitch2 = STORMDRONE_MIN_PITCH2;
	...
	if (gpGlobals->curtime > m_flNextEngineSoundTime && flEnemyDist < 48)
	{
		...
		EmitSound( "npc_stormdone.EngineNoise" );
	}
}
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::MoveExecute_Alive(float flInterval)
{
	...
	if ( (CBaseEntity*)GetEnemy() )
	{
		...
		if( flDist < STORMDRONE_CHARGE_MIN_DIST )
		{
			// Less noise up close.
			noiseScale = 2.0;
		}

		if ( IsInEffectiveTargetZone( GetEnemy() ) && flDist < STORMDRONE_CHARGE_MIN_DIST && gpGlobals->curtime > m_flNextBurstTime )
		{
			Vector vecCurrentDir = GetCurrentVelocity();
			VectorNormalize( vecCurrentDir );
			...
			{				
				...
				SetEyeState( STORMDRONE_EYE_STATE_CHARGE );
			}
		}
	...
	if ( HasPhysicsAttacker( STORMDRONE_SMASH_TIME ) && (!IsHeldByPhyscannon()) && VPhysicsGetObject() )
	{
		Vector vecCurrentVelocity;
		VPhysicsGetObject()->GetVelocity( &vecCurrentVelocity, NULL );
		float flLerpFactor = (gpGlobals->curtime - m_flLastPhysicsInfluenceTime) / STORMDRONE_SMASH_TIME;
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::SpinBlades(float flInterval)
{
	if (!m_bBladesActive)
	{
		SetBodygroup( STORMDRONE_BODYGROUP_BLADE, STORMDRONE_BODYGROUP_OFF );
		SetBodygroup( STORMDRONE_BODYGROUP_BLUR, STORMDRONE_BODYGROUP_OFF );
		m_flBladeSpeed = 0.0;
	...
	if ( IsFlyingActivity( GetActivity() ) )
	{
		...
		{
			SetBodygroup( STORMDRONE_BODYGROUP_BLADE, STORMDRONE_BODYGROUP_ON );
			SetBodygroup( STORMDRONE_BODYGROUP_BLUR, STORMDRONE_BODYGROUP_OFF );
		}
		else if (m_flBladeSpeed < 40)
		{
			SetBodygroup( STORMDRONE_BODYGROUP_BLADE, STORMDRONE_BODYGROUP_ON );
			SetBodygroup( STORMDRONE_BODYGROUP_BLUR, STORMDRONE_BODYGROUP_ON );
		}
		else
		{
			SetBodygroup( STORMDRONE_BODYGROUP_BLADE, STORMDRONE_BODYGROUP_OFF );
...
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::MoveExecute_Dead(float flInterval)
{
	if( GetWaterLevel() > 0 )
	{
...
//		UTIL_Smoke(GetAbsOrigin(), random->RandomInt(10, 15), 10);
	...
	// Limit overall speed
	// ----------------------
	LimitSpeed( -1, STORMDRONE_MAX_SPEED * 2.0 );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::Precache(void)
{
	//
	// Model.
	//
	PrecacheModel("models/manhack.mdl");
	PrecacheModel( STORMDRONE_GLOW_SPRITE );
	PropBreakablePrecacheAll( MAKE_STRING("models/manhack.mdl") );
	
	PrecacheScriptSound( "npc_stormdone.Die" );
	PrecacheScriptSound( "npc_stormdone.Bat" );
	PrecacheScriptSound( "npc_stormdone.Grind" );
	PrecacheScriptSound( "npc_stormdone.Slice" );
	PrecacheScriptSound( "npc_stormdone.EngineNoise" );
	PrecacheScriptSound( "npc_stormdone.Unpack" );
	PrecacheScriptSound( "npc_stormdone.ChargeAnnounce" );
	PrecacheScriptSound( "npc_stormdone.ChargeEnd" );
	PrecacheScriptSound( "npc_stormdone.Stunned" );

	// Sounds used on Client:
	PrecacheScriptSound( "npc_stormdone.EngineSound1" );
	PrecacheScriptSound( "npc_stormdone.EngineSound2"  );
	PrecacheScriptSound( "npc_stormdone.BladeSound" );
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::GatherEnemyConditions( CBaseEntity *pEnemy )
{
	...
	{
		SetCondition(COND_STORMDRONE_START_ATTACK);
	}
	else
	{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_STORMDRONE::MeleeAttack1Conditions( float flDot, float flDist )
{
	if ( GetEnemy() == NULL )
		return COND_NONE;
...
// Input  : *pTask - 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		// Override this task so we go for the enemy at eye level
	case TAsk_stormdrone_HOVER:
		{
			break;
		}
	...
	case TAsk_stormdrone_MOVEAT_SAVEPOSITION:
		{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::Spawn(void)
{
	Precache();
	...
	if ( HasSpawnFlags( SF_STORMDRONE_CARRIED ) )
	{
	...
	SetNoiseMod( STORMDRONE_NOISEMOD_HIDE, STORMDRONE_NOISEMOD_HIDE, STORMDRONE_NOISEMOD_HIDE );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::StartEye( void )
{
	//Create our Eye sprite
	if ( m_pEyeGlow == NULL )
	{
		m_pEyeGlow = CSprite::SpriteCreate( STORMDRONE_GLOW_SPRITE, GetLocalOrigin(), false );
	...
	if ( m_pLightGlow == NULL )
	{
		m_pLightGlow = CSprite::SpriteCreate( STORMDRONE_GLOW_SPRITE, GetLocalOrigin(), false );
...
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::PostNPCInit( void )
{
	// SetAbsVelocity( vec3_origin ); 
	m_bBladesActive = (m_spawnflags & (SF_STORMDRONE_PACKED_UP|SF_STORMDRONE_CARRIED)) ? false : true;
	BladesInit();
}

void CNPC_STORMDRONE::BladesInit()
{
	if( !m_bBladesActive )
	{
...
// Crank up the engine!
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::StartEngine( bool fStartSound )
{
	if( fStartSound )
	{
		SoundInit();
	...
	if( ( m_spawnflags & SF_STORMDRONE_PACKED_UP ) && vecVelocity.z < 0 )
	{
		// DevMsg(" POP UP \n" );
...
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::SoundInit( void )
{
	m_nEnginePitch1 = STORMDRONE_MIN_PITCH1;
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::StopLoopingSounds(void)
{
...
// Input  : pTask - 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::StartTask( const Task_t *pTask )
{
	switch (pTask->iTask)
	{	
	case TAsk_stormdrone_UNPACK:
		{
			// Just play a sound for now.
			EmitSound( "npc_stormdone.Unpack" );
	...
	case TAsk_stormdrone_FIND_SQUAD_CENTER:
		{
			if (!m_pSquad)
			{
	...
	case TAsk_stormdrone_FIND_SQUAD_MEMBER:
		{
			if (m_pSquad)
			{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::UpdateOnRemove( void )
{
	DestroySmokeTrail();
	KillSprites( 0.0 );
...
//-----------------------------------------------------------------------------
bool CNPC_STORMDRONE::HandleInteraction(int interactionType, void* data, CBaseCombatCharacter* sourceEnt)
{
...
// Output : float
//-----------------------------------------------------------------------------
float CNPC_STORMDRONE::ManhackMaxSpeed( void )
{
	if( m_flWaterSuspendTime > gpGlobals->curtime )
	{
		// Slower in water!
		return STORMDRONE_MAX_SPEED * 0.1;
	}

	if ( HasPhysicsAttacker( STORMDRONE_SMASH_TIME ) )
	{
		return STORMDRONE_NPC_BURST_SPEED;
	}

	return STORMDRONE_MAX_SPEED;
}
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::ClampMotorForces( Vector &linear, AngularImpulse &angular )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::KillSprites( float flDelay )
{
	if( m_pEyeGlow )
	{
...
//-----------------------------------------------------------------------------
bool CNPC_STORMDRONE::IsInEffectiveTargetZone( CBaseEntity *pTarget )
{
...
//			&tolerance - 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::TranslateNavGoal( CBaseEntity *pEnemy, Vector &chasePosition )
{
...
float CNPC_STORMDRONE::GetDefaultNavGoalTolerance()
{
	return GetHullWidth();
}
...
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::InputDisableSwarm( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::InputUnpack( inputdata_t &inputdata )
{
	if ( HasSpawnFlags( SF_STORMDRONE_PACKED_UP ) == false )
		return;
...
//			reason - 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::OnPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
...
//			Reason - 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::OnPhysGunDrop( CBasePlayer *pPhysGunUser, PhysGunDrop_t Reason )
{
...
void CNPC_STORMDRONE::StartLoitering( const Vector &vecLoiterPosition )
{
	//Msg("Start Loitering\n");
...
CBasePlayer *CNPC_STORMDRONE::HasPhysicsAttacker( float dt )
{
...
//-----------------------------------------------------------------------------
float CNPC_STORMDRONE::GetMaxEnginePower()
{
	if( m_bHackedByAlyx )
	{
		return 2.0f;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::UpdatePanels( void )
{
	if ( m_flEngineStallTime > gpGlobals->curtime )
	{
...
// Input  : hostile - 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::ShowHostile( bool hostile /*= true*/)
{
	...
	if ( hostile )
	{
		EmitSound( "npc_stormdone.ChargeAnnounce" );
	}
	else
	{
		EmitSound( "npc_stormdone.ChargeEnd" );
	}
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::StartBurst( const Vector &vecDirection )
{
	if ( m_flBurstDuration > gpGlobals->curtime )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::StopBurst( bool bInterruptSchedule /*= false*/ )
{
	if ( m_flBurstDuration < gpGlobals->curtime )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_STORMDRONE::SetEyeState( int state )
{
	...
	switch( state )
	{
	case STORMDRONE_EYE_STATE_STUNNED:
		{
			if ( m_pEyeGlow )
			{
				//Toggle our state
			...
			EmitSound("npc_stormdone.Stunned");

			break;
		}

	case STORMDRONE_EYE_STATE_CHARGE:
		{
			if ( m_pEyeGlow )
			{
				//Toggle our state
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_STORMDRONE::CreateVPhysics( void )
{
	if ( HasSpawnFlags( SF_STORMDRONE_CARRIED ) )
		return false;
...
//
//-----------------------------------------------------------------------------
AI_BEGIN_CUSTOM_NPC( npc_stormdone, CNPC_STORMDRONE )

	DECLARE_TASK( TAsk_stormdrone_HOVER );
	DECLARE_TASK( TAsk_stormdrone_UNPACK );
	DECLARE_TASK( TAsk_stormdrone_FIND_SQUAD_CENTER );
	DECLARE_TASK( TAsk_stormdrone_FIND_SQUAD_MEMBER );
	DECLARE_TASK( TAsk_stormdrone_MOVEAT_SAVEPOSITION );

	DECLARE_CONDITION( COND_STORMDRONE_START_ATTACK );

	DECLARE_ACTIVITY( ACT_MANHACK_UNPACK );

//=========================================================
// > SCHED_STORMDRONE_ATTACK_HOVER
//=========================================================
DEFINE_SCHEDULE
(
	SCHED_STORMDRONE_ATTACK_HOVER,
...
(
	...
	"	Tasks"
	"		TASK_STOP_MOVING							0"
	"		TASK_SET_FAIL_SCHEDULE						SCHEDULE:SCHED_STORMDRONE_SWARM_FAILURE"
	"		TAsk_stormdrone_FIND_SQUAD_CENTER				0"
	"		TAsk_stormdrone_MOVEAT_SAVEPOSITION			5"
	"	"
	"	Interrupts"
	"		COND_NEW_ENEMY"
	"		COND_SEE_ENEMY"
...
(
	...
	"		TASK_SET_FAIL_SCHEDULE						SCHEDULE:SCHED_STORMDRONE_SWARM_FAILURE"
	"		TAsk_stormdrone_FIND_SQUAD_CENTER				0"
	"		TAsk_stormdrone_MOVEAT_SAVEPOSITION			1"
	"	"
	"	Interrupts"
	"		COND_NEW_ENEMY"
	"		COND_CAN_MELEE_ATTACK1"
...
(
	...
	"		TASK_WAIT									2"
	"		TASK_WAIT_RANDOM							2"
	"		TAsk_stormdrone_FIND_SQUAD_MEMBER				0"
